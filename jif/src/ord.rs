...
pub struct OrdChunk {
    ...
    /// The offset into the phdr list within which this chunk lies
    ...
    pub(crate) fn split_by_intervals(&self, jif: &Jif) -> Vec<OrdChunk> {
        ...
        while cursor < self.last_page_addr() {
            ...
            ords.push(OrdChunk {
                ...
                phdr: jif.mapping_pheader_idx(head).expect("failed to find phdr"),
            });
        }
...
mod test {
    ...
    #[test]
    fn empty_ord() {
        let ord = OrdChunk::new(0, 0x1234, 0, DataSource::Zero, 0);
        assert_eq!(
            ord,
            OrdChunk {
                timestamp_us: 0,
    ...
    #[test]
    fn single_page_ord() {
        let ord = OrdChunk::new(0, 0x1234, 1, DataSource::Zero, 0);
        assert_eq!(
            ord,
            OrdChunk {
                timestamp_us: 0,
    ...
    #[test]
    fn multi_page_ord() {
        let ord = OrdChunk::new(0, 0x1234, 10, DataSource::Zero, 0);
        assert_eq!(
            ord,
            OrdChunk {
                timestamp_us: 0,
    ...
    fn merge_diff_sources() {
...

        {
            ...
            assert_eq!(ord, OrdChunk::new(0, 0x10000, 0x8, DataSource::Zero, 0));
        ...
        {
            let mut ord = OrdChunk::new(0, 0x19000, 0x6, DataSource::Zero, 0);

            assert!(ord.merge_page(&jif, ord_fn(0, 0x18000)));
            assert_eq!(ord, OrdChunk::new(0, 0x18000, 0x7, DataSource::Zero, 0));

            assert!(!ord.merge_page(&jif, ord_fn(0, 0x17000)));

            assert!(ord.merge_page(&jif, ord_fn(0, 0x1f000)));
            assert_eq!(ord, OrdChunk::new(0, 0x18000, 0x8, DataSource::Zero, 0));
    ...
    fn merge_same_sources() {
        ...
        {
            let mut ord = OrdChunk::new(0, 0x11000, 0xe, DataSource::Zero, 0);

            assert!(ord.merge_page(&jif, ord_fn(0, 0x10000)));
            assert_eq!(ord, OrdChunk::new(0, 0x10000, 0xf, DataSource::Zero, 0));

            assert!(ord.merge_page(&jif, ord_fn(0, 0x17000)));
            assert_eq!(ord, OrdChunk::new(0, 0x10000, 0xf, DataSource::Zero, 0));

            assert!(ord.merge_page(&jif, ord_fn(0, 0x1f000)));
            assert_eq!(ord, OrdChunk::new(0, 0x10000, 0x10, DataSource::Zero, 0));
    ...
    fn merge_with_timestamp() {
        ...
        {
            let mut ord = OrdChunk::new(100, 0x11000, 0xe, DataSource::Zero, 0);

            assert!(ord.merge_page(&jif, ord_fn(150, 0x10000)));
            assert_eq!(ord, OrdChunk::new(100, 0x10000, 0xf, DataSource::Zero, 0));

            assert!(ord.merge_page(&jif, ord_fn(50, 0x17000)));
            assert_eq!(ord, OrdChunk::new(50, 0x10000, 0xf, DataSource::Zero, 0));
